...
public class SinusoidalSway : MonoBehaviour
{
    ...
    void Sway(){
        ...
        transform.position += new Vector3(0,0,ComputeSinWave(x));

    }
    }
    float ComputeSinWave(float x){
        return movespeed + (sinwidth * Mathf.Sin( sinspeed * x ));
